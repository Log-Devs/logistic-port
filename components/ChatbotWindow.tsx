...
const ChatbotWindow: React.FC<ChatbotWindowProps> = ({ onClose }) => {
  ...
  const exportChatHistory = () => {
    const historyText = messages
      .map(
        (m) =>
          `[${m.timestamp}] ${m.role === "user" ? "You" : APP_DISPLAY_NAME}: ${
            m.content
          }`
      )
      .join("\n\n");
  ...
  return (
    <motion.div
      ...
      className={clsx(
        "fixed bottom-28 right-8 w-[450px] max-w-[95vw] z-[10001]",
    ...
    >
      ...
      <div
        className={clsx(
          "bg-background border-b border-border/40 transition-colors flex items-center justify-between px-5 py-4",
          resolvedTheme === "dark"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-lg">{BOT_NAME}</h2>
      ...
      <div
        ...
        className="flex-1 overflow-y-auto py-4 px-3 space-y-4 max-h-[60vh] scroll-smooth"
      >
        {messages.map((msg, i) => (
          <motion.div
            key={i}
          ...
          >
            {msg.role === "bot" && (
              <div className="flex-shrink-0 mt-1">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-rose-500 via-rose-400 to-rose-700 border-2 border-white shadow-md flex items-center justify-center overflow-hidden">
                  <img
                    src="/avatar-nana.png"
                    alt={`${APP_DISPLAY_NAME} Avatar`}
            ...
            <div
              className={clsx(
                "max-w-[85%] rounded-2xl p-3.5 text-sm relative group",
                msg.role === "user"
            ...
            >
              ...
              {isMessageLong(msg.content) && !expandedMessages.includes(i) && (
                <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-muted to-transparent flex items-end justify-center">
                  <button
              ...
              <div className="flex justify-between mt-1">
                <span className="text-xs opacity-60 pt-1">{msg.timestamp}</span>
            ...
            {msg.role === "user" && (
              <div className="flex-shrink-0 mt-1">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-zinc-300 to-zinc-500 border-2 border-white shadow flex items-center justify-center overflow-hidden">
                  <img
                    src="/avatar-user.png"
                    alt="User Avatar"
                    className="w-8 h-8 object-cover rounded-full border-2 border-zinc-200 shadow"
                    onError={(e) => {
        ...
        {loading && (
          ...
          >
            ...
            <div className="bg-muted rounded-2xl rounded-tl-none p-3.5 text-sm shadow-sm">
        ...
        {error && (
          <div className="p-4 rounded-lg bg-destructive/10 text-destructive-foreground text-sm border border-destructive/20">
            <p>{error}</p>
            <button
            ...
            <div className="flex flex-wrap gap-2">
              {suggestions.map((suggestion, i) => (
                <button
                  ...
                  className="bg-background border border-border rounded-full px-3 py-1.5 text-xs hover:bg-muted transition-colors"
                >
                  {suggestion}
                </button>
              ))}
      ...
      <div className="border-t p-3 bg-muted/10">
        <div className="flex gap-2 items-center">
          <input
            ...
            disabled={loading}
            className={clsx(
              "flex-1 px-4 py-2.5 rounded-full border text-sm",
